            ...
            try
            {
                ...
                foreach (DataRow row in filteredRows)
                {
                    ...
                    if (row["Img"] != DBNull.Value)
                    {
                        ...
                        string imagePath = Path.Combine("D:\\BTL C#\\BTL_W\\BTL\\Images", imageName);
        ...
        private void LoadDataGridView()
        {
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    string imagePath = Path.Combine(Application.StartupPath, "D:\\BTL C#\\BTL_W\\BTL\\Images", row["ImageUrl"].ToString());
                    if (File.Exists(imagePath))
                    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                foreach (DataGridViewRow row in dgvAdd.Rows)
                {
                    ...
                    string destinationPath = $"D:\\BTL C#\\BTL_W\\BTL\\Images\\{img}";
